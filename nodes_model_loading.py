...
class WanVideoModelLoader:
    ...
    def loadmodel(self, model, base_precision, load_device,  quantization,
        ...
        # TODO(wbai): assign load_device to transformer_load_device
        transformer_load_device = device if load_device == "main_device" else offload_device
        print(f"[WENBING DEBUG]: transformer_load_device: {transformer_load_device}")
        ...
        # ToDo(wbai): not sure why this line is not working, comment it out for now
        ...
        if not gguf:
            ...
            print(f"[WENBING DEBUG]: Complete loaded sd in device: {device}")
        else:
            gguf_reader=[]
        ...
        # ToDo(wbai): use requires_grad_(False) to be compatible with Wan2.1 example
        ...
        # ToDo(wbai): Place here for now
        # Enable XDI-T for Wan2.1
        # import types
        # from wanvideowrapper.wanvideo.distributed.xdit_context_parallel import (
        #     usp_attn_forward,
        #     usp_dit_forward,
        ...
        #         usp_attn_forward, block.self_attn) -> work in place
        # for block in transformer.vace_blocks:
        #     block.self_attn.forward = types.MethodType(
        #         usp_attn_forward, block.self_attn) -> work in place
        # transformer.forward = types.MethodType(usp_dit_forward, transformer) -> work in place
        # transformer.forward_vace = types.MethodType(usp_dit_forward_vace, transformer) -> work in place
        if dist.is_initialized():
            dist.barrier()

        comfy_model.diffusion_model = transformer
        comfy_model.load_device = transformer_load_device
        print(f"[WENBING DEBUG]: comfy_model.load_device: {comfy_model.load_device}")
        patcher = comfy.model_patcher.ModelPatcher(comfy_model, device, offload_device)
        print(f"[WENBING DEBUG]: ModelPatcher.device: {device}")
...
class WanVideoVAELoader:
    ...
    def loadmodel(self, model_name, precision, compile_args=None):
        ...
        vae_sd = load_torch_file(model_path, device=device, safe_load=True)
        ...
        # ToDo(wbai): Enable multi GPU support
